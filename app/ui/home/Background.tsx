    ...
    return (
        <motion.section >
            <motion.section
                ...
                {[0, 1, 2, 3].map((index) => (
                    <section key={index} className={`relative h-screen w-full animate-spin-slow ${index % 2 == 0 && "rotate-180"}`}>
                        <Image fill className="object-cover select-none" alt="Background" src={`/SpaceForHome.svg`}></Image>
                    </section>
                ))}
            </motion.section>
            {appear && (
